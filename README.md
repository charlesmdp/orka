...
Set `OPENAI_API_KEY` as a **Production secret** in the Cloudflare Pages project's **Settings → Variables and secrets**, then deploy. Never add the value to source control. The model is `gpt-5-mini`, Responses API, `store: false`, bounded input and output, 25-second provider timeout. Failure shows an error and offers the real Orka chat. The handoff calls `window.Orka.show(question)` after widget readiness, prefilling a draft that the visitor reviews and sends. It never sends the message automatically.
